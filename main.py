...
# -------------------
# Set ONE of these:
FILTER_BY_POSTAL_CODE = False # Set to False to use top N active stations
POSTAL_CODE = "08005"        # Only used if FILTER_BY_POSTAL_CODE is True
TOP_N_STATIONS = 100          # Only used if FILTER_BY_POSTAL_CODE is False

DATA_DIR = "data"
INFO_FILE_YEAR = 2022  # Use a year with a complete *_INFO.csv
...
def get_top_n_active_stations(data_dir, years, n):
    # Scan all *_STATIONS.csv for the given years, count activity per station
    ...
    for file in files:
        for chunk in pd.read_csv(file, usecols=["station_id", "num_bikes_available"], chunksize=500_000):
            ...
            # Activity: sum of absolute changes in num_bikes_available per station
...
# -------------------
# 7. PLOT SAMPLE STATION
# -------------------
sample_station = station_ids[0]  # Pick the first station in the list
...
# Only plot a single week to avoid memory overload
...
if not train_df.empty:
    ...
    print("⚠️ EDA skipped: train_df is empty.")
# -------------------
# 8. SAVE OUTPUTS
# -------------------
